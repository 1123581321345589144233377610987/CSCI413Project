...
big_df.drop(columns=["Unnamed: 0", "insulin_kind_L", "insulin_kind_R"], inplace=True)
...
for subject_id in subject_ids:
    ...
    # drop dates before or after pump data started
    start_ind = df["basal_dose"].first_valid_index()
    end_ind = df["basal_dose"].last_valid_index()

    df = df.loc[start_ind:end_ind]

    # set any remaining bolus nulls to 0
    df["bolus_dose"].fillna(0, inplace=True)

    # set any remaining basal nulls to average
    avg_basal = df["basal_dose"].mean()
    df["basal_dose"].fillna(avg_basal, inplace=True)

    # remove glucose nulls if asleep
    df = df[ ~(df["value"].isnull() & df["is_asleep"]) ]

    # drop dates before or after intensity data started
    start_ind = df["intensity"].first_valid_index()
    end_ind = df["intensity"].last_valid_index()

    df = df.loc[start_ind:end_ind]

    # set any remaining intensity nulls to average
    avg_intensity = df["intensity"].mean()
    df["intensity"].fillna(avg_intensity, inplace=True)
